...
struct Forecastday : Codable {
    ...
    // Gets the formatted date with a short month and timestamp.
    var formattedDate: String? {
        guard let localtime = date?.formatDate(with: .shortMonth, timeStaamp: true) else { return nil }
        return localtime
    }
    // Gets the formatted week from the date with a timestamp.
    var formattedWeek: String? {
        guard let localtime = date?.formatDate(with: .weekOnly, timeStaamp: true) else { return nil }
        return localtime
    }
